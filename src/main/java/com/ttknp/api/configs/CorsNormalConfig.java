...
*/
@Configuration
public class CorsNormalConfig implements WebMvcConfigurer {

    private static final Logger log = LoggerFactory.getLogger(CorsNormalConfig.class);

    /// it's same @CrossOrigin() , Note, it's not the same on secure cors
    @Override
    public void addCorsMappings(CorsRegistry registry) {
        log.debug("Configuring addCorsMappings (normal)");
        registry.addMapping("/register/**") // Allow all req start with /
                .allowedOrigins("http://localhost:4200","http://thitikorn-nupan.com") // Allow origins
                .allowedMethods("POST","OPTIONS") // Allow HTTP methods
